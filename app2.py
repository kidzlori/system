...
# ── Tabs ─────────────────────────────────────────────────────────────
tab_radar, tab_compare, tab_similar, tab_scatter, tab_search, tab_playstyle = st.tabs([
    "📊 Percentile Radar", "⚖️ Player Comparison", "🔍 Player Similarity",
    "📈 Scatter Plot", "🔎 Player Search", "🏟️ Team Playstyle"
])
...
with tab_scatter:
    ...
    st.markdown(
        "<p style='color:#8899aa;margin-top:-15px;'>"
        "Eksploruj zależności między statystykami — hover na kropce = szczegóły gracza</p>",
        unsafe_allow_html=True
    )

    # ── Filtry ──────────────────────────────────────────────────────────
    sc_r1c1, sc_r1c2, sc_r1c3, sc_r1c4 = st.columns([3, 1, 1, 1])
    with sc_r1c1:
    ...
    with sc_r1c2:
        sc_min = st.number_input("Min. minut", min_value=0, max_value=5000,
                                  value=500, step=100, key="sc_min")
    with sc_r1c3:
        sc_color_by = st.selectbox("Kolor wg", ["Pozycja", "Liga", "Drużyna"],
                                    key="sc_color_by")
    with sc_r1c4:
        sc_show_labels = st.checkbox("Etykiety", value=False, key="sc_labels")

    sc_r2c1, sc_r2c2 = st.columns([2, 2])
    with sc_r2c1:
        sc_pos_groups = st.multiselect(
            "Grupy pozycji (puste = wszystkie)", _SCATTER_POS_OPTIONS,
            default=[], key="sc_pos"
        )
    with sc_r2c2:
        # Druzyna — wypelniana lazily po wybraniu ligi
        _sc_team_opts = []
        if sc_leagues:
            _sc_tc = next((c for c in ['Team within selected timeframe', 'Team']
                           if c in df_raw.columns), None)
            if _sc_tc:
                _mask_lg_sc = df_raw[league_col].astype(str).apply(_league_key).isin(
                    [_league_key(l) for l in sc_leagues]
                )
                _sc_team_opts = sorted(
                    df_raw.loc[_mask_lg_sc, _sc_tc].dropna().astype(str).str.strip().unique().tolist()
                )
        sc_team_filter = st.selectbox(
            "Drużyna (opcja)", ["— wszystkie —"] + _sc_team_opts,
            key="sc_team"
        )

    st.markdown("---")

    # ── Wybór osi — PRZED przyciskiem, user może dowolnie zmieniać ──────
    _sc_all_stats = _all_scatter_stats(df_raw)
    _sc_lbl_map   = {c: _stat_label(c) for c in _sc_all_stats}

    sc_ax1, sc_ax2, sc_ax3 = st.columns(3)
    with sc_ax1:
        x_col_name = st.selectbox(
            "Oś X", _sc_all_stats,
            format_func=lambda c: _sc_lbl_map.get(c, c),
            key="sc_x"
        )
    with sc_ax2:
        y_col_name = st.selectbox(
            "Oś Y", _sc_all_stats,
            index=min(1, len(_sc_all_stats)-1),
            format_func=lambda c: _sc_lbl_map.get(c, c),
            key="sc_y"
        )
    with sc_ax3:
        _size_opts = ["— brak —"] + _sc_all_stats
        sc_size_name = st.selectbox(
            "Rozmiar bąbla (opcja)", _size_opts,
            format_func=lambda c: _sc_lbl_map.get(c, c) if c != "— brak —" else c,
            key="sc_size"
        )
    ...
    if not sc_submit:
        ...
        x_col    = x_col_name
        y_col    = y_col_name
        size_col = sc_size_name if sc_size_name != "— brak —" else None

        # ── Dataset ─────────────────────────────────────────────────────
        pos_col_sc = next((c for c in ['Primary position','Position','position','Pos']
                           if c in df_raw.columns), None)
        nc_sc = 'Full name' if 'Full name' in df_raw.columns else 'Player'
        tc_sc = next((c for c in ['Team within selected timeframe','Team']
                      if c in df_raw.columns), None)

        df_sc = df_raw.copy()
        df_sc['_main_pos'] = (df_sc[pos_col_sc].astype(str).str.split(',').str[0].str.strip()
                              if pos_col_sc else 'N/A')
        df_sc['_minutes']  = pd.to_numeric(
            df_sc.get('Minutes played', 0), errors='coerce').fillna(0)
        ...
        # Filtr druzyny
        if sc_team_filter != "— wszystkie —" and tc_sc:
            df_sc = df_sc[df_sc[tc_sc].astype(str).str.strip() == sc_team_filter]
        ...
        df_sc = df_sc.dropna(subset=['_x','_y']).reset_index(drop=True)
        ...
        if df_sc.empty:
            ...
            with hl_col1:
                all_players_sc = sorted(df_sc[nc_sc].dropna().astype(str).str.strip().unique())
                sc_highlight = st.multiselect("⭐ Zaznacz gracza/y",
            ...
            # reset_index już byl — indeksy sa 0..n-1
            color_vals  = df_sc[color_key].astype(str).fillna('N/A').reset_index(drop=True)
            ...
            x_std = float(df_sc['_x'].std()) or 1
            y_std = float(df_sc['_y'].std()) or 1
            x_range_pad = max((df_sc['_x'].max() - df_sc['_x'].min()) * 0.05, 0.01)
            y_range_pad = max((df_sc['_y'].max() - df_sc['_y'].min()) * 0.05, 0.01)
            ...
            # ── Rozmiar bąbli — tablica numpy indeksowana pozycyjnie ──
            if size_col and '_sz' in df_sc.columns:
                ...
                marker_sizes = np.clip(6 + 18*(df_sc['_sz'].values - sz_min)/sz_range, 4, 24)
            else:
                marker_sizes = np.full(len(df_sc), 8.0)
            ...
            for qx, qy, qarrow in [
                ...
                fig_sc.add_annotation(x=qx, y=qy, text=qarrow, showarrow=False,
                    font=dict(size=18, color="#2a3a4e"),
                    xanchor="center", yanchor="middle")

            # ── Traces — czysty pozycyjny indeks ─────────────────────
            for val in unique_vals:
                # Maska jako numpy bool array (pozycyjna, bez pandas index)
                mask_v = (color_vals.values == val)
                indices = np.where(mask_v)[0]   # pozycje w df_sc

                if len(indices) == 0:
                    continue

                x_vals  = df_sc['_x'].values[indices]
                y_vals  = df_sc['_y'].values[indices]
                sz_vals = marker_sizes[indices]
                ...
                for idx in indices:
                    ...
                    pos_ = str(row.get('_main_pos', '')).strip()
                    ...
                    ht = (
                        f"<b>{name}</b><br>"
                        f"{team} · {lg}<br>"
                        f"Poz: {pos_} · {mins} min<br>"
                        f"<b>{_sc_lbl_map.get(x_col,x_col)}: {xv:.2f}</b><br>"
                        f"<b>{_sc_lbl_map.get(y_col,y_col)}: {yv:.2f}</b>"
                    )
                    if size_col and '_sz' in df_sc.columns:
                        ht += f"<br>{_sc_lbl_map.get(size_col,size_col)}: {row['_sz']:.2f}"
                    hover_texts.append(ht)

                # Highlight mask (pozycyjna)
                names_v   = df_sc[nc_sc].values[indices].astype(str)
                hl_bool   = np.isin(np.char.strip(names_v), list(sc_highlight))
                norm_bool = ~hl_bool

                # Normalne punkty
                if norm_bool.any():
                    fig_sc.add_trace(go.Scatter(
                        x=x_vals[norm_bool].tolist(),
                        y=y_vals[norm_bool].tolist(),
                        mode='markers' + ('+text' if sc_show_labels else ''),
                        marker=dict(
                            color=color_map[val],
                            size=sz_vals[norm_bool].tolist(),
                            opacity=0.75,
                            line=dict(color="#0d1117", width=0.5),
                        ),
                        text=[n for n, h in zip(names_v, hl_bool) if not h] if sc_show_labels else None,
                        textposition="top center",
                        textfont=dict(size=8, color="#c8d8e8"),
                        hovertext=[t for t, h in zip(hover_texts, hl_bool) if not h],
                        hoverinfo="text",
                        hoverlabel=dict(bgcolor="#1a2a3a", font=dict(color="white", size=12),
                                        bordercolor=color_map[val]),
                        name=val, legendgroup=val, showlegend=True,
                    ))

                # Highlighted (gwiazdy)
                if hl_bool.any():
                    fig_sc.add_trace(go.Scatter(
                        x=x_vals[hl_bool].tolist(),
                        y=y_vals[hl_bool].tolist(),
                        mode='markers+text',
                        marker=dict(
                            color=color_map[val],
                            size=np.clip(sz_vals[hl_bool] + 8, 0, 32).tolist(),
                            opacity=1.0,
                            line=dict(color="#FFFFFF", width=2.5),
                            symbol='star',
                        ),
                        text=[n for n, h in zip(names_v, hl_bool) if h],
                        textposition="top center",
                        textfont=dict(size=11, color="#FFFFFF"),
                        hovertext=[t for t, h in zip(hover_texts, hl_bool) if h],
                        hoverinfo="text",
                        hoverlabel=dict(bgcolor="#1a2a3a", font=dict(color="white", size=12),
                                        bordercolor="#FFFFFF"),
                        name=f"★ {val}", legendgroup=val, showlegend=False,
                    ))

            # Labelki mediany
            x_title = _sc_lbl_map.get(x_col, x_col)
            y_title = _sc_lbl_map.get(y_col, y_col)

            fig_sc.add_annotation(x=x_med, y=y_hi, showarrow=False,
                text=f"mediana X: {x_med:.2f}",
                font=dict(size=9, color="#5a6a7a"),
                yanchor="top", xanchor="left", xshift=4)
            fig_sc.add_annotation(x=x_hi, y=y_med, showarrow=False,
            ...
            st.caption(
                ...
                + (f" · Rozmiar: *{_sc_lbl_map.get(size_col,size_col)}*" if size_col else "")
                + (f" · Drużyna: *{sc_team_filter}*" if sc_team_filter != "— wszystkie —" else "")
                + f" · Mediana X: {x_med:.2f}, Y: {y_med:.2f}"
            )

            st.plotly_chart(fig_sc, use_container_width=True)

            # ── Top 15 prawego górnego kwadrantu ─────────────────────
            with st.expander("📋 Top 15 — prawy górny kwadrant"):
                df_q1 = df_sc[(df_sc['_x'] >= x_med) & (df_sc['_y'] >= y_med)].copy()
                df_q1['_score'] = (df_q1['_x'] - x_med)/x_std + (df_q1['_y'] - y_med)/y_std
...
with tab_search:
    ...
    # Filtr kontraktów
    import datetime
    ps_only_expiring = st.radio(
        "Only expiring contracts?",
        ["No", "Yes"],
        horizontal=True,
        key="ps_only_expiring"
    )
    ps_contract_date = None
    if ps_only_expiring == "Yes":
        ps_contract_date = st.date_input(
            "Contract Expires On Or Before",
            value=datetime.date.today().replace(year=datetime.date.today().year + 1),
            min_value=datetime.date.today(),
            max_value=datetime.date.today().replace(year=datetime.date.today().year + 5),
            key="ps_contract_date",
            format="DD/MM/YYYY",
        )
    ...
    if not ps_submit:
        ...
        # 5. Kontrakty
        if ps_only_expiring == "Yes" and ps_contract_date is not None:
            _contract_col = next(
                (c for c in ['Contract expires', 'Contract Expires', 'contract_expires',
                             'Contract expiry', 'Contract Expiry', 'contract expiry']
                 if c in df_res.columns), None
            )
            if _contract_col:
                # Parsuj daty — Wyscout/Opta używają różnych formatów (YYYY, DD/MM/YYYY itd.)
                def _parse_contract(v):
                    if pd.isna(v):
                        return pd.NaT
                    v = str(v).strip()
                    # Samo rok np. "2025" lub "2026"
                    if v.isdigit() and len(v) == 4:
                        return pd.Timestamp(f"{v}-06-30")  # koniec sezonu
                ...
                df_res['_contract_dt'] = df_res[_contract_col].apply(_parse_contract)
                cutoff = pd.Timestamp(ps_contract_date)
                df_res = df_res[
                    df_res['_contract_dt'].notna() &
                    (df_res['_contract_dt'] <= cutoff)
                ]
            else:
                st.warning("⚠️ Brak kolumny 'Contract expires' w danych. Filtr kontraktów pominięty.")
        ...
        if n_found == 0:
            ...
            for _, r in df_res.iterrows():
                ...
                _cc = next((c for c in ['Contract expires', 'Contract Expires',
                            ...
                            if c in df_res.columns), None)
                if _cc:
                    row_d['Kontrakt'] = str(r.get(_cc, '—')).strip()
...
PLAYSTYLE_DIR = 'Stat_Files'

@st.cache_data(show_spinner=False)
def scan_playstyle_files(folder):
    """Zwraca {season_label: [filepath, ...]} z folderu Stat_Files."""
    import glob as _glob
    seasons = {}
    if not os.path.isdir(folder):
        return seasons
    for fp in sorted(_glob.glob(os.path.join(folder, '*.csv'))):
        fname = os.path.basename(fp)
        # Wyciagnij sezon z nazwy pliku np. "Bundesliga_25-26.csv" → "25-26"
        m = _re.search(r'(\d{2}[-/]\d{2}|\d{4}[-/]\d{2}|\d{4}[-/]\d{4})', fname)
        season = m.group(1) if m else 'Unknown'
        seasons.setdefault(season, []).append(fp)
    return seasons

@st.cache_data(show_spinner=False)
def load_playstyle_data(filepaths: tuple) -> pd.DataFrame:
    """Wczytuje i łączy pliki CSV z Stat_Files. Zwraca surowe wiersze (per mecz)."""
    dfs = []
    for fp in filepaths:
        try:
            df = pd.read_csv(fp, encoding='utf-8-sig')
            # Dodaj kolumne z nazwa pliku/ligi
            league_name = _re.sub(r'[_\-]?\d{2}[-/]?\d{2,4}', '', os.path.splitext(os.path.basename(fp))[0])
            league_name = league_name.strip('_ ')
            df['_source_league'] = league_name
...
def _num(df, col):
    """Bezpieczna konwersja kolumny do float."""
    if col in df.columns:
        return pd.to_numeric(df[col], errors='coerce')
    return pd.Series(0.0, index=df.index)

def build_playstyle_dimensions(df_team: pd.Series) -> dict:
    """
    Buduje 9 wymiarów Playstyle z zagregowanych statystyk druzyny.
    df_team = wiersz ze srednimi per mecz.
    Zwraca dict {dimension_name: raw_value}.
    """
    def v(col):
        val = df_team.get(col, 0.0)
        try:
            return float(val) if pd.notna(val) else 0.0
        except Exception:
            return 0.0

    # Composites — suma standaryzowanych składników wyliczana pozniej
    # Tutaj zwracamy surowe wartosci do percentylowania
    return {
        # ── ATAK ──────────────────────────────────────────────────────
        'open_play_threat':  v('Open Play xG') + v('xT') + v('Shots') * 0.05,
        'set_piece_threat':  v('Set Piece xG') + v('Corners') * 0.03 + v('Passes into Box') * 0.01,
        'chance_quality':    (v('xG') / max(v('Shots'), 1)) + v('npxG') * 0.3,
        # ── OBRONA ────────────────────────────────────────────────────
        'pressing_intensity': (1 / max(v('PPDA'), 0.1)) * 10 + v('High Recoveries') * 0.1 + v('On-Ball Pressure') * 0.003,
        'open_play_defense':  1 / max(v('Open Play xGA') + v('xT Against') * 0.3, 0.01),
        'set_piece_defense':  1 / max(v('Set Piece xGA'), 0.01),
        # ── STYL GRY ──────────────────────────────────────────────────
        'possession_control': v('Possession') * 0.4 + v('Field Tilt') * 0.4 + v('Game Control') * 0.002,
        'progression_style':  v('Passes in Opposition Half') * 0.01 + v('xT') * 0.5 - v('Avg Pass Height') * 0.01,
        'wide_play':          v('Crosses') * 0.15 + v('Field Tilt') * 0.01,
    }

PLAYSTYLE_META = {
    # key: (label, group, group_color, description)
    'open_play_threat':   ('Open Play\nThreat',   'Atak',     '#00BFFF', 'Open Play xG + xT + Shots'),
    'set_piece_threat':   ('Set Piece\nThreat',   'Atak',     '#00BFFF', 'Set Piece xG + Corners + Passes into Box'),
    'chance_quality':     ('Chance\nQuality',     'Atak',     '#00BFFF', 'xG/Shot + npxG'),
    'pressing_intensity': ('Pressing\nIntensity', 'Obrona',   '#FF6B35', '1/PPDA + High Recoveries + On-Ball Pressure'),
    'open_play_defense':  ('Open Play\nDefense',  'Obrona',   '#FF6B35', '1/(Open Play xGA + xT Against)'),
    'set_piece_defense':  ('Set Piece\nDefense',  'Obrona',   '#FF6B35', '1/Set Piece xGA'),
    'possession_control': ('Possession\nControl', 'Styl gry', '#2DCA72', 'Possession + Field Tilt + Game Control'),
    'progression_style':  ('Progression\nStyle',  'Styl gry', '#2DCA72', 'Passes in Opp Half + xT - Avg Pass Height'),
    'wide_play':          ('Wide\nPlay',          'Styl gry', '#2DCA72', 'Crosses + Field Tilt'),
}

DIM_ORDER = list(PLAYSTYLE_META.keys())

def build_playstyle_radar(team_name, pct_vals, all_teams_pct, season_label):
    """
    Rysuje Plotly Scatterpolar dla playstyle radaru.
    pct_vals: dict {dim: percentile 0-100}
    all_teams_pct: dict {team: {dim: percentile}} — do showowania innych druzyn
    """
    labels   = [PLAYSTYLE_META[d][0] for d in DIM_ORDER]
    r_vals   = [pct_vals.get(d, 50) for d in DIM_ORDER]
    theta    = labels + [labels[0]]
    r_closed = r_vals + [r_vals[0]]

    # Kolory grup
    group_colors = [PLAYSTYLE_META[d][2] for d in DIM_ORDER]

    fig = go.Figure()

    # Siatki referencyjne
    for lvl, alpha in [(25, 0.06), (50, 0.10), (75, 0.08), (100, 0.0)]:
        fig.add_trace(go.Scatterpolar(
            r=[lvl]*len(DIM_ORDER) + [lvl],
            theta=theta,
            mode='lines',
            line=dict(color=f'rgba(100,120,140,{alpha*3:.2f})', width=0.7),
            hoverinfo='skip', showlegend=False,
        ))

    # Kolorowe segmenty (jeden trace per wymiar dla kolorow)
    for i, dim in enumerate(DIM_ORDER):
        color = PLAYSTYLE_META[dim][2]
        fig.add_trace(go.Scatterpolar(
            r=[0, r_vals[i], r_vals[(i+1) % len(DIM_ORDER)], 0],
            theta=[labels[i], labels[i], labels[(i+1) % len(DIM_ORDER)], labels[i]],
            mode='lines',
            fill='toself',
    ...
    # Główny polygon
    hover_r = [
        f"<b>{PLAYSTYLE_META[d][0].replace(chr(10),' ')}</b><br>"
        f"Percentyl: <b>{pct_vals.get(d,50):.0f}</b><br>"
        f"<span style='color:#8899aa'>{PLAYSTYLE_META[d][3]}</span>"
        for d in DIM_ORDER
    ] + [f"<b>{PLAYSTYLE_META[DIM_ORDER[0]][0].replace(chr(10),' ')}</b>"]
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        marker=dict(
            size=[12]*len(r_closed),
            color=[PLAYSTYLE_META[d][2] for d in DIM_ORDER] + [PLAYSTYLE_META[DIM_ORDER[0]][2]],
    ...
    # Pierscien 50 (mediana ligi)
    fig.add_trace(go.Scatterpolar(
        r=[50]*len(DIM_ORDER) + [50],
        theta=theta,
        mode='lines',
    ...
    fig.update_layout(
        ...
        title=dict(
            text=f"<b>{team_name}</b>  —  Playstyle Profile  <span style='color:#5a6a7a;font-size:13px'>({season_label})</span>",
            font=dict(color='#e8edf2', size=16),
            x=0.5,
        ),
    )
    return fig
...
with tab_playstyle:
    st.title("🏟️ Team Playstyle Profiles")
    st.markdown(
        "<p style='color:#8899aa;margin-top:-15px;'>"
        "Profil stylu gry drużyny — 9 wymiarów: Atak · Obrona · Styl gry</p>",
        unsafe_allow_html=True
    )

    if not os.path.isdir(PLAYSTYLE_DIR):
        st.error(f"❌ Folder **'{PLAYSTYLE_DIR}'** nie istnieje. Dodaj pliki CSV do folderu Stat_Files.")
        st.stop()

    # ── Sezony ────────────────────────────────────────────────────────
    ps_seasons = scan_playstyle_files(PLAYSTYLE_DIR)
    if not ps_seasons:
        st.error(f"❌ Brak plików CSV w folderze **'{PLAYSTYLE_DIR}'**.")
        st.stop()

    # Użyj sezonu wybranego w Radar Filters (sidebar)
    ps_season_label = sel_season_label
    _ps_key = next(
        (k for k in ps_seasons if _normalize_season(k.replace('/', '-')) == season_key),
        sorted(ps_seasons.keys(), reverse=True)[0]
    )

    with st.spinner("Wczytywanie danych..."):
        df_ps_raw = load_playstyle_data(tuple(sorted(ps_seasons[_ps_key])))

    if df_ps_raw.empty:
        st.error("Brak danych dla wybranego sezonu.")
        st.stop()

    # Kolumna druzyny
    team_col_ps = next(
        (c for c in ['Team', 'team', 'Club', 'club'] if c in df_ps_raw.columns), None
    )
    if not team_col_ps:
        st.error("Brak kolumny 'Team' w danych.")
        st.stop()

    # ── Agregacja: srednia per mecz per druzyna ───────────────────────
    num_cols_ps = [c for c in df_ps_raw.columns
                   if c not in [team_col_ps, 'Match', 'Date', '_source_league', '_source_file']
                   and pd.api.types.is_numeric_dtype(
                       pd.to_numeric(df_ps_raw[c], errors='coerce')
                   )]

    df_ps_raw[num_cols_ps] = df_ps_raw[num_cols_ps].apply(
        pd.to_numeric, errors='coerce'
    )
    df_ps_agg = (
        df_ps_raw.groupby(team_col_ps, as_index=False)[num_cols_ps].mean()
    )
    df_ps_agg = df_ps_agg.dropna(subset=[team_col_ps])

    # ── Oblicz raw composite dla wszystkich druzyn ────────────────────
    composites = {team: {} for team in df_ps_agg[team_col_ps]}
    for _, row in df_ps_agg.iterrows():
        team = row[team_col_ps]
        dims = build_playstyle_dimensions(row)
        composites[team] = dims

    # Zamien na DataFrame
    df_comp = pd.DataFrame.from_dict(composites, orient='index')
    df_comp.index.name = 'Team'
    ...
    for col in DIM_ORDER:
        if col in df_pct.columns:
            series = df_pct[col]
            # Czyste percentyle: ranga / n * 100
            df_pct[col] = series.rank(pct=True) * 100

    all_teams_sorted = sorted(df_ps_agg[team_col_ps].dropna().unique().tolist())

    # ── Wybór druzyny ────────────────────────────────────────────────
    ps_team_sel = st.selectbox(
        "🏟️ Select Team", all_teams_sorted, key="ps_team_sel"
    )

    n_matches_team = len(df_ps_raw[df_ps_raw[team_col_ps] == ps_team_sel])
    n_teams_total  = len(all_teams_sorted)

    st.caption(
        f"Sezon: **{ps_season_label}** · "
        f"Mecze w bazie: **{n_matches_team}** · "
        f"Liczba drużyn: **{n_teams_total}**"
    )

    st.markdown("---")

    # ── Radar ────────────────────────────────────────────────────────
    if ps_team_sel not in df_pct.index:
        st.warning("Brak danych dla wybranej drużyny.")
    else:
        pct_row = df_pct.loc[ps_team_sel]
        pct_vals_team = {d: float(pct_row.get(d, 50)) for d in DIM_ORDER}

        fig_ps = build_playstyle_radar(
            ps_team_sel, pct_vals_team, {}, ps_season_label
        )
        st.plotly_chart(fig_ps, use_container_width=True)

        # ── Tabela wymiarów ──────────────────────────────────────────
        st.markdown("### 📊 Szczegóły wymiarów")

        raw_row  = df_comp.loc[ps_team_sel] if ps_team_sel in df_comp.index else {}
        tbl_rows = []
        for dim in DIM_ORDER:
            meta  = PLAYSTYLE_META[dim]
            label = meta[0].replace('\n', ' ')
            group = meta[1]
            desc  = meta[3]
            pct   = pct_vals_team.get(dim, 0)
            raw   = float(raw_row.get(dim, 0)) if hasattr(raw_row, 'get') else 0.0

            # Ranking w lidze
            if dim in df_pct.columns:
                rank = int(df_pct[dim].rank(ascending=False).loc[ps_team_sel]) \
                       if ps_team_sel in df_pct.index else '—'
            else:
                rank = '—'
            ...
            tbl_rows.append({
                ...
                'Ranking':    f"{rank}/{n_teams_total}",
                'Składowe':   desc,
            })

        df_tbl_ps = pd.DataFrame(tbl_rows)
        st.dataframe(
            df_tbl_ps,
            use_container_width=True,
            hide_index=True,
            height=360,
            column_config={
                'Percentyl': st.column_config.ProgressColumn(
                    'Percentyl', format='%.1f',
                    min_value=0, max_value=100,
                ),
                'Grupa': st.column_config.TextColumn('Grupa', width='small'),
                'Ranking': st.column_config.TextColumn('Ranking', width='small'),
            },
        )

        # ── Porownanie z innymi druzyna (opcja) ──────────────────────
        with st.expander("⚖️ Porównaj z inną drużyną"):
            ps_team2 = st.selectbox(
                "Wybierz drużynę do porównania",
                [t for t in all_teams_sorted if t != ps_team_sel],
                key="ps_team2"
            )

            if ps_team2 and ps_team2 in df_pct.index:
                pct_row2 = df_pct.loc[ps_team2]
                pct_vals2 = {d: float(pct_row2.get(d, 50)) for d in DIM_ORDER}

                labels2   = [PLAYSTYLE_META[d][0] for d in DIM_ORDER]
                theta2    = labels2 + [labels2[0]]
                r1_closed = [pct_vals_team.get(d, 50) for d in DIM_ORDER] + [pct_vals_team.get(DIM_ORDER[0], 50)]
                r2_closed = [pct_vals2.get(d, 50) for d in DIM_ORDER] + [pct_vals2.get(DIM_ORDER[0], 50)]

                fig_cmp2 = go.Figure()

                for r_vals_c, name_c, color_c, fill_c in [
                    (r2_closed, ps_team2,   '#FF6B35', 'rgba(255,107,53,0.12)'),
                    (r1_closed, ps_team_sel,'#00BFFF', 'rgba(0,191,255,0.15)'),
                ]:
                    fig_cmp2.add_trace(go.Scatterpolar(
                        r=r_vals_c, theta=theta2,
                        mode='lines+markers', fill='toself',
                        fillcolor=fill_c,
                        line=dict(color=color_c, width=2.5),
                        marker=dict(size=8, color=color_c,
                                    line=dict(color='#0d1117', width=1.5)),
                        name=name_c, showlegend=True,
                        hoverinfo='skip',
                    ))
                ...
                fig_cmp2.update_layout(
                    ...
                    title=dict(
                        text=f"<b>{ps_team_sel}</b> <span style='color:#5a6a7a'>vs</span> <b>{ps_team2}</b>",
                        font=dict(color='#e8edf2', size=15), x=0.5,
                    ),
                )
        ...
        st.markdown("---")
        st.markdown("### 📈 Style Development Over Seasons")

        # ── Zbierz dane ze wszystkich sezonow dla wybranej druzyny ────
        @st.cache_data(show_spinner=False)
        def _load_all_seasons_dimensions(team_name: str, playstyle_dir: str):
            """Liczy wymiary Playstyle dla danej druzyny w kazdym sezonie."""
            import glob as _glob2
            result_rows = []
            all_fps = sorted(_glob2.glob(os.path.join(playstyle_dir, '*.csv')))
            for fp in all_fps:
                fname = os.path.basename(fp)
                m = _re.search(r'(\d{2}[-/]\d{2}|\d{4}[-/]\d{2}|\d{4}[-/]\d{4})', fname)
                if not m:
                    continue
                raw_season = m.group(1)
                league_name = _re.sub(r'[_\-]?\d{2}[-/]?\d{2,4}', '', os.path.splitext(fname)[0]).strip('_ ')
                try:
                    df_s = pd.read_csv(fp, encoding='utf-8-sig')
                except Exception:
                    continue
                team_col = next((c for c in ['Team','team','Club','club'] if c in df_s.columns), None)
                if not team_col:
                    continue
                df_team = df_s[df_s[team_col] == team_name]
                if df_team.empty:
                    continue
                num_cols = [c for c in df_team.columns
                            if c not in [team_col, 'Match', 'Date', '_source_league', '_source_file']
                            and pd.api.types.is_numeric_dtype(pd.to_numeric(df_team[c], errors='coerce'))]
                df_team = df_team.copy()
                df_team[num_cols] = df_team[num_cols].apply(pd.to_numeric, errors='coerce')
                row_mean = df_team[num_cols].mean()
                dims = build_playstyle_dimensions(row_mean)
                dims['_season'] = raw_season
                dims['_league'] = league_name
                result_rows.append(dims)
            return result_rows

        season_rows = _load_all_seasons_dimensions(ps_team_sel, PLAYSTYLE_DIR)

        if season_rows:
            df_evo = pd.DataFrame(season_rows)
            # Sortuj wg sezonu
            df_evo = df_evo.sort_values('_season')
            # Normalize 0-1 per dimension across all values
            dim_cols = [d for d in DIM_ORDER if d in df_evo.columns]
            for dc in dim_cols:
                mn, mx = df_evo[dc].min(), df_evo[dc].max()
                if mx > mn:
                    df_evo[dc] = (df_evo[dc] - mn) / (mx - mn)
                else:
                    df_evo[dc] = 0.5

            # X-axis labels: league + season
            df_evo['_label'] = df_evo.apply(
                lambda r: f"{r['_league']}\n{r['_season']}", axis=1
            )

            # Kolory linii
            LINE_COLORS = [
                '#00BFFF','#FF6B35','#2DCA72','#FFD700','#FF69B4',
                '#9B59B6','#1ABC9C','#E74C3C','#F39C12',
            ]
            dim_display = {d: PLAYSTYLE_META[d][0].replace('\n', ' ') for d in DIM_ORDER}

            fig_evo = go.Figure()
            for idx, dim in enumerate(dim_cols):
                color_l = LINE_COLORS[idx % len(LINE_COLORS)]
                fig_evo.add_trace(go.Scatter(
                    x=df_evo['_label'],
                    y=df_evo[dim],
                    mode='lines+markers',
                    name=dim_display[dim],
                    line=dict(color=color_l, width=2),
                    marker=dict(size=7, color=color_l,
                                line=dict(color='#0d1117', width=1.5)),
                    hovertemplate=f'<b>{dim_display[dim]}</b><br>Sezon: %{{x}}<br>Wartość: %{{y:.2f}}<extra></extra>',
                ))
            ...
            fig_evo.update_layout(
                ...
                height=440,
                margin=dict(t=50, b=100, l=60, r=180),
                title=dict(
                    text=f'<b>{ps_team_sel}</b> Style Evolution',
                    font=dict(color='#e8edf2', size=14), x=0.5,
                ),
                hovermode='x unified',
            )
            st.plotly_chart(fig_evo, use_container_width=True)
        else:
            st.info(f"Brak danych historycznych dla **{ps_team_sel}** w folderze {PLAYSTYLE_DIR}.")
        ...
        @st.cache_data(show_spinner=False)
        def _compute_all_team_profiles(playstyle_dir: str):
            """Zwraca DataFrame: Team, League, Season + 9 wymiarow (raw) dla kazdego (team,season)."""
            import glob as _glob3
            rows = []
            for fp in sorted(_glob3.glob(os.path.join(playstyle_dir, '*.csv'))):
                fname = os.path.basename(fp)
                m = _re.search(r'(\d{2}[-/]\d{2}|\d{4}[-/]\d{2}|\d{4}[-/]\d{4})', fname)
                raw_season = m.group(1) if m else 'Unknown'
                league_name = _re.sub(r'[_\-]?\d{2}[-/]?\d{2,4}', '', os.path.splitext(fname)[0]).strip('_ ')
                try:
                    df_s = pd.read_csv(fp, encoding='utf-8-sig')
                except Exception:
                ...
                if not team_col:
                    continue
                num_cols = [c for c in df_s.columns
                            if c not in [team_col, 'Match', 'Date', '_source_league', '_source_file']
                            and pd.api.types.is_numeric_dtype(pd.to_numeric(df_s[c], errors='coerce'))]
                df_s[num_cols] = df_s[num_cols].apply(pd.to_numeric, errors='coerce')
                for team, grp in df_s.groupby(team_col):
                    row_mean = grp[num_cols].mean()
                    dims = build_playstyle_dimensions(row_mean)
                    dims['Team'] = team
                    dims['League'] = league_name
                    dims['Season'] = raw_season
                    rows.append(dims)
            return pd.DataFrame(rows) if rows else pd.DataFrame()

        with st.spinner("Obliczanie podobieństwa..."):
            df_all_profiles = _compute_all_team_profiles(PLAYSTYLE_DIR)

        if not df_all_profiles.empty and ps_team_sel in df_all_profiles['Team'].values:
            dim_cols_sim = [d for d in DIM_ORDER if d in df_all_profiles.columns]
            # Normalize columns 0-1 across entire pool
            df_sim = df_all_profiles.copy()
            for dc in dim_cols_sim:
                mn, mx = df_sim[dc].min(), df_sim[dc].max()
                if mx > mn:
                    df_sim[dc] = (df_sim[dc] - mn) / (mx - mn)
                else:
                    df_sim[dc] = 0.5

            # Reference vector: current selected team in current _ps_key season
            ref_mask = (df_sim['Team'] == ps_team_sel) & (df_sim['Season'] == _ps_key)
            if not ref_mask.any():
                # Fallback: any season for selected team
                ref_mask = df_sim['Team'] == ps_team_sel
            ref_vec = df_sim.loc[ref_mask, dim_cols_sim].mean().values

            # Cosine similarity
            def cosine_sim(row):
                a = row[dim_cols_sim].values.astype(float)
                dot = np.dot(a, ref_vec)
                norm = (np.linalg.norm(a) * np.linalg.norm(ref_vec))
                return dot / norm if norm > 0 else 0.0

            df_sim['Similarity'] = df_all_profiles.apply(cosine_sim, axis=1)

            # Remove exact match row(s) of the selected team in current season
            exclude = (df_sim['Team'] == ps_team_sel) & (df_sim['Season'] == _ps_key)
            df_top = (
                df_sim[~exclude]
                .sort_values('Similarity', ascending=False)
                .head(20)
                [['Team', 'League', 'Season', 'Similarity']]
                .reset_index(drop=True)
            )
            df_top['Similarity'] = (df_top['Similarity'] * 100).round(1)
            ...
            st.info("Niewystarczające dane do obliczenia podobieństwa.")
